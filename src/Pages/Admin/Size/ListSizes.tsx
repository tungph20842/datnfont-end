...
import { getSize ,deleteSize} from "../../../api/size";

type Props = {};
const { Search } = Input;

const ListSizes = (props: Props) => {
const [sizes, setSizes] = useState<ISize[]>([]);
const sorte = sizes.reverse()
const [searchKeyword, setSearchKeyword] = useState('');
  ...
  const columns = [
    ...
    },
    {
        ...
        dataIndex: "status",
        key: "status",
        render:(status: any, record: ISize)=>(
  ...
  const filteredSizes = useMemo(() => {
    return sorte.filter((size) =>
      size.name.toLowerCase().includes(searchKeyword.toLowerCase())
    );
  }, [sorte, searchKeyword]);
  return (
    ...
    <Table dataSource={filteredSizes} columns={columns} />
